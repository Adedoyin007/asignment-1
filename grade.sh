...
if command -v git >/dev/null 2>&1 && git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    ...
    [[ "$commits" -ge 5 ]] && pass "Git has at least 5 commits" || \
    fail "Git has fewer than 5 commits"

    branches=$(git for-each-ref --format='%(refname:short)' refs/heads 2>/dev/null | \
    grep -vE '^(main|master)$' | wc -l | tr -d ' ')

    [[ "$branches" -ge 1 ]] && pass "At least one non-main local branch exists" || \
